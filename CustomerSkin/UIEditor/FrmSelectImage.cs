...
namespace CustomerSkin
{
    public partial class FrmSelectImage : Form
    {
        ...
        {
            string[] nameList = System.Enum.GetNames(typeof(CustomerSkin.FontIcons));
            var lst = nameList.ToList();
            lst.Sort();
            foreach (var item in lst)
            {
                CustomerSkin.FontIcons icon = (CustomerSkin.FontIcons)Enum.Parse(typeof(CustomerSkin.FontIcons), item);
                Label lbl = new Label();
                lbl.AutoSize = false;
                ...
                lbl.Text = "       " + item;
                lbl.Image = CustomerSkin.FontImages.GetImage(icon, 32, Color.FromArgb(255, 77, 59));
        ...
        void lbl_DoubleClick(object sender, EventArgs e)
        {
            ...
            CustomerSkin.FontIcons icon = (CustomerSkin.FontIcons)lbl.Tag;
            int intSize = ucTextBoxEx1.InputText.ToInt();
            if (intSize <= 0)
                intSize = 32;
            SelectImage = CustomerSkin.FontImages.GetImage(icon, intSize, txtForeColor.BackColor, txtBackcolor.BackColor == Color.White ? Color.Empty : txtBackcolor.BackColor);
