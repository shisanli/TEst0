...
namespace CustomerSkin.SkinControl
{
    internal class ScrollBarManager : NativeWindow, IDisposable
    {
            ...
            Orientation direction)
        {
            ...
            IntPtr hdc = Win32.NativeMethods.GetDC(maskHWnd);
            try
            {
            ...
            {
                Win32.NativeMethods.ReleaseDC(maskHWnd, hdc);
                tempDc.Dispose();
            }
        }
        ...
        {
            RECT rect = new RECT();
            Win32.NativeMethods.GetWindowRect(scrollBarHWnd, ref rect);
            Win32.NativeMethods.OffsetRect(ref rect, -rect.Left, -rect.Top);
        ...
        private SCROLLBARINFO GetScrollBarInfo(IntPtr hWnd, uint objid)
        {
            ...
            Win32.NativeMethods.GetScrollBarInfo(hWnd, objid, ref sbi);
            return sbi;
        }
        ...
        private ScrollBarHistTest ScrollBarHitTest(IntPtr hWnd)
        {
            ...
            RECT tp = rect;
            Win32.NativeMethods.GetCursorPos(ref point);
            Win32.NativeMethods.ScreenToClient(hWnd, ref point);

            if (Direction == Orientation.Horizontal)
            {
                if (Win32.NativeMethods.PtInRect(ref rect, point))
                {
                    // left arrow
                    tp.Right = arrowCx;
                    if (Win32.NativeMethods.PtInRect(ref tp, point))
                    {
