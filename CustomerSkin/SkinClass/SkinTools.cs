...
using CustomerSkin.Imaging;

namespace CustomerSkin.SkinClass
{
    public class SkinTools
    {
        ...
        {
            int Rgn = Win32.NativeMethods.CreateRoundRectRgn(0, 0, ctrl.ClientRectangle.Width + 1, ctrl.ClientRectangle.Height + 1, RgnRadius, RgnRadius);
            Win32.NativeMethods.SetWindowRgn(ctrl.Handle, Rgn, true);
        }
           ...
           bool redraw)
        {
            ...
            Win32.NativeMethods.GetWindowRect(hWnd, ref bounds);
            ...
            if (roundStyle != RoundStyle.None)
            {
                    ...
                    rect, radius, roundStyle, true))
                {
                    using (Region region = new Region(path))
                    {
                        path.Widen(Pens.White);
                        region.Union(path);
                        IntPtr hDc = Win32.NativeMethods.GetWindowDC(hWnd);
                        try
                        {
                            using (Graphics g = Graphics.FromHdc(hDc))
                            {
                                Win32.NativeMethods.SetWindowRgn(hWnd, region.GetHrgn(g), redraw);
                            }
                        }
                        finally
                        {
                            Win32.NativeMethods.ReleaseDC(hWnd, hDc);
                        }
                    }
                }
            }
        ...
        public static void CursorClick(int x, int y)
        {
            ...
            //恢复一次鼠标点击
            Win32.NativeMethods.mouse_event(MOUSEEVENTF_LEFTDOWN, x * 65536 / 1024, y * 65536 / 768, 0, 0);
            Win32.NativeMethods.mouse_event(MOUSEEVENTF_LEFTUP, x * 65536 / 1024, y * 65536 / 768, 0, 0);
        }
